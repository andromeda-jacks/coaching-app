...
const AI_PROVIDERS = {
  ...
  gemini:   { label:"Google Gemini (AI Studio) ✅ 브라우저 OK", baseUrl:"https://generativelanguage.googleapis.com/v1beta/openai", model:"gemini-2.0-flash", authStyle:"bearer", placeholder:"AIza...", group:"브라우저 직접 OK", cors:true },
...
3. 한국어 학술 문체, 우수작의 문체·구조를 참고하되 표절하지 않기
4. 분량: 전체 25쪽 중 이 섹션은 ${section.pages}
5. 마크다운 형식 (## 소제목, 표 등)
6. 데이터가 없는 항목은 [여기에 데이터 입력] 플레이스홀더로 표시`;
}
    ...
    <div>
      ...
      <div style={sCard}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div>
            <div style={{ fontSize:14, fontWeight:700, color:C.text }}>연구계획서 (7쪽)</div>
            <div style={{ fontSize:11, color:C.sub }}>제출 기한: 3.23~4.2 · [서식5] 준수</div>
          </div>
          <button style={sBtn("#059669","#fff",true)} onClick={generatePlan} disabled={genLoading==="plan"}>
            {genLoading==="plan" ? <Loader size={12} style={{animation:"spin 1s linear infinite"}}/> : <Zap size={12}/>}
            {(st.sections.plan?.content) ? " 재생성" : " AI 생성"}
          </button>
        </div>
        {st.sections.plan?.content && (
          <div style={{ marginTop:8 }}>
            <textarea value={st.sections.plan.content} rows={8}
